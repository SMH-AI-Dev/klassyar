...
export default function CreateBalloon_pop() {
  ...
  const addQuestion = () => {
    setActivity(prev => ({
      ...
      content: {
        ...prev.content,
        questions: [...prev.content.questions, { question: "", answer: "", wrongAnswers: ["", ""] }]
      }
    }));
  };
  ...
  const updateWrongAnswer = (qIndex, aIndex, value) => {
    setActivity(prev => {
      const newQuestions = [...prev.content.questions];
      newQuestions[qIndex].wrongAnswers[aIndex] = value;
      return {
        ...prev,
        content: { ...prev.content, questions: newQuestions }
      };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-gray-800">ترکاندن بادکنک 🎈</h1>
              <p className="text-sm md:text-base text-gray-600">بادکنک‌های صحیح را بترکان</p>
            </div>
          </div>
          <Button
            onClick={handleSave}
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="مثال: 8"
                      className="rounded-xl clay-element bg-green-50"
                    />
                  </div>
                  <div>
                    <Label>پاسخ‌های اشتباه ✗</Label>
                    <div className="space-y-2">
                      {q.wrongAnswers.map((wrong, aIndex) => (
                        <Input
                          key={aIndex}
                          value={wrong}
                          onChange={(e) => updateWrongAnswer(qIndex, aIndex, e.target.value)}
                          placeholder={`پاسخ غلط ${aIndex + 1}`}
                          className="rounded-xl clay-element bg-red-50"
                        />
                      ))}
                    </div>
                  </div>
        ...
        <Button
          onClick={addQuestion}
          variant="outline"
          className="w-full mt-6 h-16 rounded-2xl clay-element border-2 border-dashed border-violet-300 hover:bg-violet-50"
        >
