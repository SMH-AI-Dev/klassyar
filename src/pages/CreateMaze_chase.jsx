...
export default function CreateMaze_chase() {
  ...
  const addQuestion = () => {
    setActivity(prev => ({
      ...
      content: {
        ...prev.content,
        questions: [...prev.content.questions, { question: "", options: ["", "", "", ""], correctAnswer: 0 }]
      }
    }));
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-gray-800">تعقیب در مارپیچ 🏃</h1>
              <p className="text-sm md:text-base text-gray-600">فرار از هیولا با پاسخ صحیح</p>
            </div>
          </div>
          <Button onClick={handleSave} disabled={createMutation.isPending}
            className="w-full sm:w-auto bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 text-white rounded-xl clay-element"
          >
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <Input value={activity.title} onChange={(e) => setActivity({ ...activity, title: e.target.value })}
                placeholder="مثال: فرار از هیولا" className="rounded-xl clay-element"
              />
            </div>
            <div>
              <Label>توضیحات</Label>
        ...
        <Button onClick={addQuestion} variant="outline"
          className="w-full mt-6 h-16 rounded-2xl clay-element border-2 border-dashed border-indigo-300 hover:bg-indigo-50"
        >
