...
export default function EditMatching() {
  ...
  const addPair = () => {
    setActivity({
      ...activity,
      content: {
        pairs: [...activity.content.pairs, { left: "", right: "" }]
      }
    });
  };
          ...
          <Card className="clay-element bg-white border-0">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-bold text-gray-800">جفت‌ها</h3>
                <Button
                  onClick={addPair}
                  variant="outline"
                  size="sm"
                ...
                {activity.content.pairs.map((pair, index) => (
                  ...
                  >
                    <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <Label className="text-sm text-gray-600">سمت چپ {index + 1}</Label>
                        <Input
                          value={pair.left}
                          onChange={(e) => updatePair(index, 'left', e.target.value)}
                          placeholder="متن سمت چپ..."
                          className="mt-2 rounded-xl clay-element"
                        />
                      </div>
                      <div>
                        <Label className="text-sm text-gray-600">سمت راست {index + 1}</Label>
                        <Input
                          value={pair.right}
                          onChange={(e) => updatePair(index, 'right', e.target.value)}
                          placeholder="متن سمت راست..."
                          className="mt-2 rounded-xl clay-element"
                        />
                      </div>
                    </div>
