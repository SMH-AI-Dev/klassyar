...
export default function EditUnjumble() {
  ...
  const handleSave = () => {
    ...
    const validWords = activity.content.words.filter(w => w.word.trim());
  ...
  const addWord = () => {
    setActivity({
      ...activity,
      content: {
        words: [...activity.content.words, { word: "", hint: "" }]
      }
    });
  };
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-gray-800">
              ویرایش بازی مرتب‌سازی حروف
            </h1>
          </div>
          <Button
            onClick={handleSave}
            disabled={updateMutation.isPending}
            className="bg-gradient-to-r from-teal-500 to-green-500 hover:from-teal-600 hover:to-green-600 text-white rounded-xl clay-element"
          >
          ...
          <Card className="clay-element bg-white border-0">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-bold text-gray-800">کلمات</h3>
                <Button
                  onClick={addWord}
                  variant="outline"
                  size="sm"
              ...
              <div className="space-y-4">
                {activity.content.words.map((wordObj, index) => (
                  <motion.div
                    key={index}
                  ...
                  >
                    <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <Label className="text-sm text-gray-600">کلمه {index + 1}</Label>
                        <Input
                          value={wordObj.word}
                      ...
                      <div>
                        <Label className="text-sm text-gray-600">راهنما (اختیاری)</Label>
                        <Input
                          value={wordObj.hint}
