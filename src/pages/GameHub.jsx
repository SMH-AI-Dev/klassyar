...
const categories = ["همه", "ریاضی", "علوم", "تاریخ", "جغرافیا", "هوش", "دقت", "حافظه", "خلاقیت"];
...
export default function GameHub() {
  ...
  const filtered = gameLibrary.filter(g => {
    const matchCat = activeCat === "همه" || g.category === activeCat;
    const matchSearch = g.title.includes(search) || g.description.includes(search);
    return matchCat && matchSearch;
  });

  return (
    <div className="min-h-screen p-4 sm:p-8 bg-gradient-to-br from-purple-50 via-pink-50 to-indigo-50" dir="rtl">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="text-gray-500 text-center mb-8">۵۰ بازی متنوع برای یادگیری و سرگرمی</p>
          ...
          {categories.map(cat => (
            <button
              ...
              className={`px-4 py-2 rounded-2xl text-sm font-medium transition-all clay-element ${
                activeCat === cat ? "bg-purple-600 text-white" : "bg-white/80 text-gray-700 hover:bg-purple-50"
              }`}
            >
              {cat}
            </button>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-3 py-1 rounded-xl text-xs font-medium bg-purple-50 text-purple-600">
                      {game.category}
                    </span>
                  </div>
                </div>
